...
namespace Backend.Controllers
{
    ...
    public class SearchController : ControllerBase
    {
        ...
        {
            if (request.List == null || request.List.Length <= 0) return BadRequest("List cannot be null or empty.");

            LinearSearch linearSearch = new LinearSearch();
            linearSearch.Search(request.List.ToList(), request.Target);
        ...
        public IActionResult BinarySearch([FromBody] SearchRequest request)
        {
            ...
            binarySearch.Search(request.List.ToList(), request.Target);
